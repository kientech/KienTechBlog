...
function Header({ toggleTheme, theme }) {
  ...
  const handleSearch = async () => {
    if (searchQuery) {
      ...
      navigate(`/blogs/search-results?query=${searchQuery}`);
    }
  };
  ...
  return (
    <>
      {/* mobile header */}
      <div className="md:hidden px-2  text-blue-600 relative">
        ...
        {/* Dropdown Menu */}
        <div
          className={`fixed top-0 left-0 min-h-screen w-64 bg-white rounded-tr-md rounded-br-md text-gray-800 shadow-lg transform transition-transform duration-300 ease-in-out z-50 ${
      ...
      {/* desktop header */}
      <div className="sticky top-0 left-0 right-0 z-10 md:block hidden">
                  ...
                  <div className="absolute right-0 mt-2 w-48 bg-white border rounded-lg shadow-lg z-10">
                    <ul>
                      {user.email === "admin@gmail.com" ? (
                        <Link
                          to={"/admin/dashboard"}
                          className="block px-4 py-2 hover:bg-gray-100 cursor-pointer rounded-lg"
                        >
                          Admin Panel
                        </Link>
                      ) : (
